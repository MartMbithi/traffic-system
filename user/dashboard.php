...
<?php
    include ("../connection.php");
    $license_id = $_SESSION['license_id'];
    $result = mysqli_query($conn,"SELECT SUM(issued_fines_total_amount) From issued_fines WHERE issued_fines_license_id='$license_id' AND issued_fines_status='pending'");
    $row = mysqli_fetch_array($result);
    $totalPending = $row[0];
?>
<?php
    include ("../connection.php");
    $license_id = $_SESSION['license_id'];
    $result = mysqli_query($conn,"SELECT SUM(issued_fines_total_amount) From issued_fines WHERE issued_fines_license_id='$license_id' AND issued_fines_status='paid'");
    $row = mysqli_fetch_array($result);
    $totalPaid = $row[0];
?>
<!-- PHP Code end here -->
...
<?php
    include ("../connection.php");
    $license_id = $_SESSION['license_id'];
    $pendingcountQuery = mysqli_query($conn, "SELECT issued_fines_no FROM issued_fines WHERE issued_fines_license_id='$license_id' AND issued_fines_status='pending'");
    $pendingCount = mysqli_num_rows($pendingcountQuery);

    $paidcountQuery = mysqli_query($conn, "SELECT issued_fines_no FROM issued_fines WHERE issued_fines_license_id='$license_id' AND issued_fines_status='paid'");
...
<?php
    ...
    $license_id = $_SESSION['license_id'];
    $year = date("Y");
    $jan = mysqli_query($conn, "SELECT issued_fines_no FROM issued_fines WHERE issued_fines_license_id='$license_id' AND MONTH(issued_fines_date) = 01 AND YEAR(issued_fines_issued_date) = '$year'");
    $janVal = mysqli_num_rows($jan);

    $feb = mysqli_query($conn, "SELECT issued_fines_no FROM issued_fines WHERE issued_fines_license_id='$license_id' AND MONTH(issued_fines_date) = 02 AND YEAR(issued_fines_date) = '$year'");
    $febVal = mysqli_num_rows($feb);

    $march = mysqli_query($conn, "SELECT issued_fines_no FROM issued_fines WHERE issued_fines_license_id='$license_id' AND MONTH(issued_fines_date) = 03 AND YEAR(issued_fines_date) = '$year'");
    $marchVal = mysqli_num_rows($march);

    $april = mysqli_query($conn, "SELECT issued_fines_no FROM issued_fines WHERE issued_fines_license_id='$license_id' AND MONTH(issued_fines_date) = 04 AND YEAR(issued_fines_date) = '$year'");
    $aprilVal = mysqli_num_rows($april);

    $may = mysqli_query($conn, "SELECT issued_fines_no FROM issued_fines WHERE issued_fines_license_id='$license_id' AND MONTH(issued_fines_date) = 05 AND YEAR(issued_fines_date) = '$year'");
    $mayVal = mysqli_num_rows($may);

    $june = mysqli_query($conn, "SELECT issued_fines_no FROM issued_fines WHERE issued_fines_license_id='$license_id' AND MONTH(issued_fines_date) = 06 AND YEAR(issued_fines_date) = '$year'");
    $juneVal = mysqli_num_rows($june);

    $july = mysqli_query($conn, "SELECT issued_fines__no FROM issued_fines WHERE issued_fines_license_id='$license_id' AND MONTH(issued_fines_date) = 07 AND YEAR(issued_fines_date) = '$year'");
    $julyVal = mysqli_num_rows($july);

    $august = mysqli_query($conn, "SELECT issued_fines_no FROM issued_fines WHERE issued_fines_license_id='$license_id' AND MONTH(issued_fines_date) = 08 AND YEAR(issued_fines_date) = '$year'");
    $augustVal = mysqli_num_rows($august);

    $september = mysqli_query($conn, "SELECT issued_fines_no FROM issued_fines WHERE issued_fines_license_id='$license_id' AND MONTH(issued_fines_date) = 09 AND YEAR(issued_fines_date) = '$year'");
    $sepVal = mysqli_num_rows($september);

    $october = mysqli_query($conn, "SELECT issued_fines_no FROM issued_fines WHERE issued_fines_license_id='$license_id' AND MONTH(issued_fines_date) = 10 AND YEAR(issued_fines_date) = '$year'");
    $octVal = mysqli_num_rows($october);

    $november = mysqli_query($conn, "SELECT issued_fines_no FROM issued_fines WHERE issued_fines_license_id='$license_id' AND MONTH(issued_fines_date) = 11 AND YEAR(issued_fines_date) = '$year'");
    $novVal = mysqli_num_rows($november);

    $december = mysqli_query($conn, "SELECT issued_fines_no FROM issued_fines WHERE issued_fines_license_id='$license_id' AND MONTH(issued_fines_date) = 12 AND YEAR(issued_fines_date) = '$year'");
